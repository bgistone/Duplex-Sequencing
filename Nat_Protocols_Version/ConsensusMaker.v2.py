...
usage: consensus_maker.py [-h] [--infile INFILE] [--tag_file tag_file] [--outfile OUTFILE]
						[--interval INTERVAL_FILE] [--rep_filt REP_FILT]
...
optional arguments:
	...
	--tag_file tag_file     prefix of output tagcounts file and tagstats file(prefix.*.tagcount, prefix.*.tagstat)
	--outfile OUTFILE     output BAM file
	--interval INTERVAL_FILE	interval file for excluding alignments
...
def print_read(read_in):
	sys.stderr.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (read_in.qname, read_in.flag, read_in.tid, read_in.pos, read_in.mapq, read_in.cigar, read_in.mrnm, read_in.mpos, read_in.isize, read_in.seq, read_in.qual, read_in.tags))
...
def load_interval(interval_file):
	in_file = open(interval_file,'r')
	for line in in_file:
		tup = line.split('\t')
		for index in range(tup[1],tup[2]+1):
			interval_dict[tup[0]+":"+str(index)] = 1;

def main():
	# Parameters to be input.
	parser = ArgumentParser()
	parser.add_argument("--infile", action="store", dest="infile", help="input BAM file", required=True)
	parser.add_argument("--tag_file",  action="store",  dest="tag_file", help="prefix of output tagcounts file",
						default='sys.stdout', required=True)
	#parser.add_argument("--tag_stats",  action="store",  dest="tag_stats", help="prefix of output tag_stats file",
	...
	#parser.add_argument('--interval', type=str, action = "store", dest='interval',help="interval file for exclude alignment")
	...
	out_LCC_bam_file = pysam.Samfile(o.outfile.replace(".bam", "_LCC.bam"), "wb", template=in_bam_file)
	...
	window_position = 0
	#interval_dict = defaultdict(lambda: 0) # Initialize the interval dictionary
	read_dict = {}  # Initialize the read dictionary
	pos_dict = defaultdict(lambda: 0) # Initialize the pos dictionary
	#tag_dict = defaultdict(lambda: 0) # Initialize the tag dictionary
	postag_R1_dict = defaultdict(lambda: 0) # Initialize the tag(with chr:pos-R1) dictionary
	postag_RF_dict = defaultdict(lambda: 0) # Initialize the tag(with chr:pos-RF) dictionary
	consensus_dict = {}

	#load_interval(interval_file)
...
# Start going through the input BAM file, one position at a time.
	for line in bam_entry:
		...
		if read_one is True:
			window_position -= 1
		while (read_window[window_position % 2].pos == read_window[(window_position-1) % 2].pos and file_done is False and read_one is False) or read_one is True:
			if read_number_count % o.rOut == 0:
				timeout = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
				try:
					sys.stderr.write( str(timeout) + "\tReads processed:" + str(read_number_count) + "\t" + str(read_window[window_position % 2].reference_name) + "\t" + str(read_window[window_position % 2].pos)+"\n")
				except:
					sys.stderr.write( str(timeout) + "\tReads processed:" + str(read_number_count) + "\n")
			try:
				if 0 < len(samtags):
				...
				#postag_R1_dict[tag] += 1
				if read_window[window_position % 2].is_read1 is True:
					# Based on tag only
					tag += ":1"
					try:
						# Based on position only
						pos = str(read_window[window_position % 2].reference_name) + ":" + str(read_window[window_position % 2].pos) + ":" + str(read_window[window_position % 2].next_reference_name) + ":" + str(read_window[window_position % 2].mpos)
						pos_dict[pos] += 1
						# Based on position & Tag(R1)
						postag_R1 = tag + ":" + str(read_window[window_position % 2].reference_name) + ":" + str(read_window[window_position % 2].pos)
						postag_R1_dict[postag_R1] += 1
						# Based on position & tag(RF)
						postag_RF = tag + ":" + pos
						postag_RF_dict[postag_RF] += 1
					except:
						print read_number_count
				elif read_window[window_position % 2].is_read2 is True:
					tag += ":2"
				else:
					tag = ":se"
			except:
			...
			# Check if the given read is good data and store into read_dict
			...
			if read_one is False:
				...
				read_one = False
		else:#postion not equal or  end of bam file
			...
			for dict_tag in read_dict.keys():  # Extract sequences to send to the consensus maker
				...
				for cigar_string in read_dict[dict_tag][6].keys():  # Determine the most common cigar string
					...
					if read_dict[dict_tag][6][cigar_string][0] >= max_cigar_count:
						max_cigar_count = read_dict[dict_tag][6][cigar_string][0]
						max_cigar = cigar_string

				#max_cigar = max(cigar_string_set)

				#if cigar_string_set[max_cigar] >= o.minmem:
				if max_cigar_count >= o.minmem:
					if max_cigar_count <= o.maxmem:
						consenuses_made += 1
						consensus, fam_size = consensus_maker(read_dict[dict_tag][6][max_cigar][2:], o.cut_off, o.read_length)
					else:
						consenuses_made += 1
						consensus, fam_size = consensus_maker(random.sample(read_dict[dict_tag][6][max_cigar][2:], o.maxmem), o.cut_off, o.read_length)
					# Output other alignments as LCC
					for cigar_string in read_dict[dict_tag][6].keys():
						if cigar_string != max_cigar:
							for n in xrange(2, len(read_dict[dict_tag][6][cigar_string][2:])):#output every reads which belong to dict_tag and non-max_cigar
								a = pysam.AlignedRead()
					...
					if (consensus.count("N")/ float(len(consensus)) <= o.Ncut_off and 'n' in o.filt) \
						...
						# Maybe there is a bug here, because different moleculars may own the same tag
	...
	pos_file = open( o.tag_file + ".pos.count","w")
	pos_file.write ( "\n".join(["%s\t%d" % (SMI, pos_dict[SMI]) for SMI in sorted(pos_dict.keys(),key=lambda x: pos_dict[x], reverse=True)] ))
	postag_R1_file = open( o.tag_file+".postag.R1.count", "w" )
	postag_R1_file.write ( "\n".join(["%s\t%d" % (SMI, postag_R1_dict[SMI]) for SMI in sorted(postag_R1_dict.keys(), key=lambda x: postag_R1_dict[x], reverse=True ) ] ))

	postag_RF_file = open( o.tag_file + ".postag.RF.count","w")
	postag_RF_file.write ( "\n".join(["%s\t%d" % (SMI, postag_RF_dict[SMI]) for SMI in sorted(postag_RF_dict.keys(), key=lambda x: postag_RF_dict[x], reverse=True ) ] ))

	pos_file.close()
	postag_R1_file.close()
	postag_RF_file.close()

	tag_stats(o.tag_file+".pos.count", o.tag_file+".pos.stats")
	tag_stats(o.tag_file+".postag.R1.count", o.tag_file+".postag.R1.stats")
	tag_stats(o.tag_file+".postag.RF.count", o.tag_file+".postag.RF.stats")

	pysam.sort("-o", o.outfile.replace("sscs.bam", "sort.sscs.bam"), o.outfile)
